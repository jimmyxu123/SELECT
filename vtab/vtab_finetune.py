...
def finetune_eval(
    ...
    train_dataloader, test_dataloader, NUM_CLASSES = get_data_loader(
        DATASET, "vtab_ds", INPUT_SIZE, BATCH_SIZE
    )
    model.train()
    setattr(
        model,
        finetune_layer,
        nn.Linear(getattr(model, finetune_layer).in_features, NUM_CLASSES),
    )
    model = to_device(model, device)
    loss_fn = nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(model.parameters(), lr=LR)
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=DECAY_STEPS, gamma=DECAY_GAMMA
    )
    finetune_model = train_loop(
        train_dataloader, model, loss_fn, optimizer, scheduler, MAX_STEPS
    )
