...
def parse_args():
    ...
    parser.add_argument("--model", type=str, required=True, help="Path to the pretrained model file.")
...
def test():
    if args.model:
        ...
        model = timm.create_model('resnet50', pretrained=True, pretrained_cfg=pretrained_cfg)
    else:
        # Load the default pretrained model if no path is provided
        model = timm.create_model('resnet50', pretrained=True)
    test_all_datasets(model, "Timm Resnet50", finetune_layer = "fc", LR = 0.01, MAX_STEPS = 1000, DECAY_STEPS = 300, DECAY_GAMMA = 0.1, MOMENTUM = 0.9, BATCH_SIZE = 64, INPUT_SIZE = 64)
