...
def extract_feature_pipeline(args, do_train=False, do_val=False):
    ...
    if do_train:
        if args.train_data_type == "imagefolder":
            dataset_train = ReturnIndexDataset(
                args.train_data_path, transform=transform
            )
            ...
            dataset_train = (
                wds.WebDataset(args.train_data_path, resampled=False, shardshuffle=True)
                .decode("pil", handler=utils.log_and_continue)
                .to_tuple("jpg", "json", handler=utils.log_and_continue)
                .map(preprocess, handler=utils.log_and_continue)
            )
            breakpoint()
            data_loader_train = torch.utils.data.DataLoader(
                dataset_train,
            ...
            assert (
                args.dataset_n_samples is not None
            ), "Please provide the number of samples in the dataset"
        elif args.train_data_type == "hfds":
            dataset_train = ReaderHfds(
                root=args.dump_train_features
                if args.dump_train_features is not None
                else args.load_train_features,
                name=args.train_data_path,
                split="train",
                download=False,
            )
            data_loader_train = torch.utils.data.DataLoader(
    ...
    if do_train:
        ...
        train_features, train_labels = extract_features(
            ...
            data_loader_train,
            args.use_cuda,
            dl_size=args.dl_size,
            ds_size=args.dataset_n_search,
            subsample_size=args.dataset_n_samples,
        )
    if do_val:
    ...
    if train_features is not None and dist.get_rank() == 0:
        ...
        torch.save(
            train_features.cpu(),
            os.path.join(args.dump_train_features, "trainfeat.pth"),
        )
        torch.save(
            train_labels.cpu(),
            os.path.join(args.dump_train_features, "trainlabels.pth"),
        )
    if test_features is not None and dist.get_rank() == 0:
        os.makedirs(args.dump_val_features, mode=0o777, exist_ok=True)
        torch.save(
            test_features.cpu(), os.path.join(args.dump_val_features, "testfeat.pth")
        )
        torch.save(
            test_labels.cpu(), os.path.join(args.dump_val_features, "testlabels.pth")
        )
...
def extract_features(
    ...
    for samples, label in metric_logger.log_every(data_loader, 10, iter_len=dl_size):
        ...
        index = torch.arange(
            ...
            (batch_count * samples.shape[0]) + samples.shape[0],
        ...
        if args.save_images and batch_count % 10 == 0:
            ...
            with open(
                os.path.join(output_dir, "train-batch-%d.txt" % batch_count), "w"
            ) as f:
    ...
    assert features.size(0) == ret_labels.size(
        0
    ), "Features and labels have different sizes, {} vs {}".format(
        features.size(0), ret_labels.size(0)
    )
    ...
    if subsample_size is not None:
        features, ret_labels = utils.resample_features(
            features, ret_labels, subsample_size
        )
    ...
    if subsample_size is not None:
        ret_labels, features = utils.regularize_label_count(
            ret_labels, features, label_freq, thresh=subsample_size // len(label_freq)
        )
    else:
        ret_labels, features = utils.regularize_label_count(
            ret_labels, features, label_freq, thresh=ds_size // len(label_freq)
        )
    label_freq = torch.bincount(ret_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_images", action="store_true", help="Save images in the output folder"
    )
    ...
    if args.load_train_features is not None:
        train_features = torch.load(
            os.path.join(args.load_train_features, "trainfeat.pth")
        )
        train_labels = torch.load(
            os.path.join(args.load_train_features, "trainlabels.pth")
        )
    else:
        train_features, _, train_labels, _ = extract_feature_pipeline(
            args, do_train=True
        )
