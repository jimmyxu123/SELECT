...
def test_all_models():
    for model_name in MODEL_LIST:
        model = timm.create_model('resnet50', pretrained=True, pretrained_cfg = {'file': model_name})
        test_all_datasets(model, model_name[13:], finetune_layer = "fc", LR = 0.01, MAX_STEPS = 1000, DECAY_STEPS = 300, DECAY_GAMMA = 0.1, MOMENTUM = 0.9, BATCH_SIZE = 64, INPUT_SIZE = 64)
    original_resnet_50 = timm.create_model('resnet50', pretrained=True)
    test_all_datasets(original_resnet_50, "Timm Resnet50", finetune_layer = "fc", LR = 0.01, MAX_STEPS = 1000, DECAY_STEPS = 300, DECAY_GAMMA = 0.1, MOMENTUM = 0.9, BATCH_SIZE = 64, INPUT_SIZE = 64)
